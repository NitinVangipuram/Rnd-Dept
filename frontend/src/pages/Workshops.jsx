...
                // Refined logic to handle multiline quoted fields
                const rawLines = csvText.split('\n');
                let dataLines = [];
                let tempLine = '';
                let inQuote = false;

                // Combine lines that are part of a quoted field
                for (const line of rawLines) {
                    const trimmedLine = line.trim();
                    if (trimmedLine === '' && !inQuote) continue; // Skip empty lines unless inside a quote

                    // Count quotes in the current line to check for open/close quotes
                    const quoteCount = (trimmedLine.match(/"/g) || []).length;
                    
                    // If the number of quotes is odd, it means a quote is opened or closed
                    if (quoteCount % 2 !== 0) {
                        inQuote = !inQuote;
                    }
...
                    
                    // If not in a quote, we have a complete record
                    if (!inQuote) {
                        dataLines.push(tempLine);
                        tempLine = ''; // Reset for the next record
                    } else {
                        tempLine += '\n'; // Add newline back for multiline fields
                    }
                }

                if (dataLines.length === 0) {
                    throw new Error("No data found in the CSV file after processing.");
                }

                // Step 2: Parse headers from the first processed line
...
                // Step 3: Parse data from the rest of the processed lines
